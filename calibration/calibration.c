...
int main(int argc, char** argv)
{
  ...
  if (miss_min_i > hit_max_i+4)
    fprintf(stderr,"Flush+Reload possible!\n");
  else if (miss_min_i > hit_max_i+2)
    fprintf(stderr,"Flush+Reload probably possible!\n");
  else if (miss_min_i < hit_max_i+2)
    fprintf(stderr,"Flush+Reload maybe not possible!\n");
  else
    fprintf(stderr,"Flush+Reload not possible!\n");
  size_t min = -1UL;
  size_t min_i = 0;
